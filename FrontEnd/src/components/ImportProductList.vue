...
const pagedProducts = computed(() => {
  ...
  let filtered = props.mode === 'importGoods'
...
function submitImportOrder() {
  if (!stockImportData.value || !stockImportData.value.stockImportDetail || stockImportData.value.stockImportDetail.length === 0) {
  ...
  const dataToSend = {
    ...
    stockImportDetail: stockImportData.value.stockImportDetail.map(({ product_id, quantity, unitPrice, totalPrice }) => ({
  ...
  axios.post('http://localhost:8083/StockImport/create', dataToSend, {
...
watch(() => [filterProducts.value.length, props.mode], () => {
  currentPage.value = 1;
});
...
<template>
  ...
  <div class="container-part">
    ...
    <div class="list-product">
      ...
      <div v-for="product in pagedProducts" :key="product.product_id" class="list-product-row">
        <input type="checkbox" v-model="selectedProduct" class="checkbox display-jus-ali" :value="product.product_id">
  ...
  <div class="createOrder-Container">
    <div class="createOrder" @click="submitImportOrder">
      Tạo đơn nhập
    </div>
  </div>
</template>
