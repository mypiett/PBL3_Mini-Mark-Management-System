...
onMounted(() => {
  ...
  axios
    .get('http://localhost:8083/api/products/categories')
    .then((response) => {
  ...
  axios
    .get('http://localhost:8083/api/products/suppliers')
    .then((response) => {
...
watch([searchQuery, selectedType, selectedTag, selectedSupplier, selectedFilter], () => {
  currentPage.value = 1;
});
      ...
      <div v-if="displayEdit_3" class="classify">
        <div class="search">
          ...
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Tìm kiếm theo mã sản tên sản phẩm"
            style="padding: 10px"
          />
        </div>
            ...
            <div v-if="showFilterDropdown" class="dropdown">
              <ul>
                <li
                  ...
                  :key="item"
                  @click="selectFilter('filter', item)"
        ...
        <div>
          ...
          <button @click="clearFilters" class="save-filter-btn active">Bỏ bộ lọc</button>
        </div>
      </div>
    </div>
          ...
          <tr v-for="product in pagedProducts" :key="product.product_id">
            ...
            <td v-if="displayEdit_2 && displayEdit">
              <div class="action-buttons">
                <router-link :to="`/editProduct/${product.product_id}`" class="edit-btn"
