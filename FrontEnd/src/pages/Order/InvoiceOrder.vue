...
<template>
  <div class="container">
    ...
    <div class="invoice-container" v-if="order">
      <router-link to="/invoice" class="back-button" style="text-decoration: none; font-size: 30px"
        >⬅️</router-link
      >
      <div class="header">
        ...
        <p>
          Ngày đặt hàng: <strong>{{ formattedOrderDate }}</strong>
        </p>
      </div>
      ...
      <div class="info">
        ...
        <p>
          ...
          {{ order.employee?.name || 'Không có thông tin nhân viên' }}
        </p>
      </div>
      ...
      <table class="product-table">
        ...
        <tbody>
          <tr v-for="item in order.order_detail" :key="item.product_id">
            ...
            <!-- hoặc Linh gọi API product để lấy tên -->
      ...
      <div class="totals">
        <p>
          Tổng cộng: <span>{{ totalBeforeDiscount.toLocaleString() }} đ</span>
        </p>
        <p v-if="order.voucher">
          Giảm giá: <span>{{ order.voucher }}%</span>
        </p>
        <p v-if="order.voucher">
          Tổng sau giảm: <span>{{ order.total_amount }} đ</span>
        </p>
      </div>
