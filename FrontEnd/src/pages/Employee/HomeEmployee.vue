...
const editEmployeeById = async () => {
  try {
    const response = await axios.patch(
      `http://localhost:8083/api/employees/${editEmployee.value.employee_id}`,
      editEmployee.value
    );
    const index = employees.value.findIndex(
      (employee) => employee.employee_id === editEmployee.value.employee_id
    );
    if (index != -1) {
...
<template>
  <div class="temp">
    ...
    <div class="main-right">
      ...
      <div
        class="overlay"
        v-if="showEditEmployeeForm || showAddEmployeeForm || showConfirmModal"
            ...
            <tbody>
              <tr v-for="(employee, index) in filteredEmployees" :key="employee.employee_id">
                ...
                <td>
                  ...
                  <button class="btn-delete" @click="deleteEmployee(employee.employee_id)">
                    Xóa
                  </button>
