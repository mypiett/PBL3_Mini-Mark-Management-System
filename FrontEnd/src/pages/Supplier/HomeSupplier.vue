...
const editSupplierById = async () => {
  try {
    const response = await axios.patch(
      `http://localhost:8083/api/suppliers/${editSupplier.value.supplier_id}`,
      editSupplier.value
    );
    const index = suppliers.value.findIndex(
      (supplier) => supplier.supplier_id === editSupplier.value.supplier_id
    );
    if (index != -1) {
...
<template>
  <div class="temp">
    ...
    <div class="main-right">
      ...
      <div
        class="overlay"
        v-if="showEditSupplierForm || showAddSupplierForm || showConfirmModal"
        @click="showEditSupplierForm = false"
      ></div>
      <div class="container">
            ...
            <tbody>
              <tr v-for="(supplier, index) in filteredSuppliers" :key="supplier.supplier_id">
                ...
                <td>
                  ...
                  <button class="btn-delete" @click="deleteSupplier(supplier.supplier_id)">
                    Xóa
                  </button>
