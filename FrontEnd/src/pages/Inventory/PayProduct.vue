...
onMounted(async () => {
  try {
    // 1. Lấy chi tiết nhập
    const resDetails = await axios.get(`http://localhost:8083/StockImportDetails/details/${importId}`);
    ...
    const resProductIds = await axios.get(`http://localhost:8083/StockImportDetails/productIds/${importId}`);
...
const selectedDetail = computed(() => {
  return importDetails.value.find(detail =>
...
const handleComplete = async () => {
  ...
  const finalReason = selectedReason.value === 'Khác' ? otherReasonText.value : selectedReason.value;


  try {
    // Gọi API xóa chi tiết nhập hàng
    const deleteRes = await axios.delete(`http://localhost:8083/StockImportDetails/delete/${importDetailId}`);
    if (deleteRes.status === 200 || deleteRes.status === 204) {
      console.log('Xoá chi tiết thành công');


      // Gọi API cập nhật lại tổng tiền đơn nhập
      const updateTotalRes = await axios.put(`http://localhost:8083/StockImport/update-total/${importId}`);
