...
public interface StockImportRepository extends JpaRepository<StockImport, String> {
  @Query(
      "SELECT s FROM StockImport s WHERE FUNCTION('MONTH', s.importDate) = :month AND FUNCTION('YEAR', s.importDate) = :year")
  List<StockImport> findByMonthAndYear(@Param("month") int month, @Param("year") int year);

  @Query(
      "SELECT s FROM StockImport s WHERE MONTH(s.importDate) = :month AND YEAR(s.importDate) = :year AND s.status = 'ACTIVE'")
