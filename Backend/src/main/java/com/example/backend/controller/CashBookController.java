...
public class CashBookController {
    ...
    // GET chi tiết sổ quỹ theo tháng và năm
    ...
    // PUT cập nhật sổ quỹ theo ID
    @PutMapping("/{id}")
    public ResponseEntity<CashBook> updateCashBook(
            @PathVariable Long id,
            @RequestBody CashBook updatedData) {
...
//         System.out.println("Tổng số phiếu nhập: " + allImports.size());

//         Set<String> allKeys = new HashSet<>();

//         for (Orders o : allOrders) {
//             if (o.getOrder_date() == null) continue;
...
//             if (i.getImportDate() == null) continue;
...
public ResponseEntity<String> generateAuto() {
    try {
        ...
        // Gom tất cả cặp tháng-năm từ orders và imports
        Set<String> allKeys = new HashSet<>();
        for (Orders o : allOrders) {
        ...
        // Tránh trùng: lưu các key đã xử lý
        Set<String> processed = new HashSet<>();

        for (String key : allKeys) {
    if (processed.contains(key)) continue; // tránh xử lý trùng
    processed.add(key);
